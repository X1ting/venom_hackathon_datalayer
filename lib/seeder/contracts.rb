module Seeder
  class Contracts
    class << self
      ...
      def seed_everscale_contracts!
        per_page = 30
        page = 0
        first_response = Requester::Scanner::Everscale::Mainnet.contracts_list(page: 0, per_page: 1)
        total_amount = first_response["total_count"]

        while (page * per_page) < total_amount do
          response = Requester::Scanner::Everscale::Mainnet.contracts_list(page: page, per_page: per_page)
          response["data"].each do |data|
            create_everscale_contract!(data)
          end
          page +=1
        end
      end

      def create_everscale_contract!(data)
        create_contract!(data, "everscale", "mainnet", Requester::Scanner::Everscale::Mainnet.base_uri)
      end

      def create_venom_contract!(data)
        create_contract!(data, "venom", "devnet", Requester::Scanner::Venom::Devnet.base_uri)
      end

      def create_contract!(data, blockchain, network, uploaded_by)
